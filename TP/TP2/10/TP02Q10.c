...
bool testaFim(char palavra[]){ //FIM  
     bool teste = false;
...
void trocaElem(Personagem listaPersonagem[], int posicaoAtual, int posicaoMenor){
...
void ordenaPorSelec(Personagem listaPersonagem[], int index, int tamanhoVetor, int *ptrComp, int *ptrTroca){
    if (index == (tamanhoVetor - 2)) {
        *ptrComp += 1;
        if (strcmp(listaPersonagem[index].nome, listaPersonagem[index + 1].nome) > 0){
            *ptrComp += 1;
            *ptrTroca += 1;
            trocaElem(listaPersonagem, index, index + 1);
        }
    }else{
        int menor = index;
        ...
        if (menor != index){
            *ptrTroca += 1;
            trocaElem(listaPersonagem, index, menor);
        }
        ordenaPorSelec(listaPersonagem, index + 1, tamanhoVetor, ptrComp, ptrTroca);
    }
}
...
int main(void){
    ...
    ordenaPorSelec(listaPersonagem, 0, contadorTamanho, ptrComp, ptrMov);
